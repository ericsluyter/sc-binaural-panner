// Modified in 2015 by Eric Sluyter for use in BinauralPannerGUI
...
BParaSpace {
  ...
  *new { arg w, bounds;
  ...
  createNode {arg x, y, color, refresh=true;
    ...
    paraNodes.add(BParaNode.new(x+0.5, y+0.5, fillcolor, bounds, nodeCount, nodeSize));
  ...
  setNodeStates_ {arg array; // array with [locs, connections, color, size, string]
    if(array[0].isNil == false, {
      paraNodes = List.new;
      array[0].do({arg loc;
        paraNodes.add(BParaNode.new(loc.x, loc.y, fillcolor, bounds, nodeCount));
        nodeCount = nodeCount + 1;
      })
    });
